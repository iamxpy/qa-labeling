...
from bert_utils import compute_output_arrays, compute_compact_split_input_arrays

np.set_printoptions(suppress=True)

SAVE_MODELS = False
BERT_SEQUENCE_LEN = 512  # DO NOT change this
LONG_SENTENCE_MAX_LEN = 700
max_short_sentence_len = 63
num_split = int(ceil(LONG_SENTENCE_MAX_LEN / (BERT_SEQUENCE_LEN - max_short_sentence_len)))
learning_rate = 3e-5
epochs = 10
batch_size = 8 // (2 * num_split)
...
def multi_split_bert_model():
    ...
    # mask 掉没有信息的输出，因为bert对于全0输入的输出不是0,不符合预期
    sequence_output = tf.cast(tf.expand_dims(bert_masks, -1), tf.float32) * sequence_output
...
# 计算multi bert 输入
inputs = []
inputs += compute_compact_split_input_arrays(df_train, question_input_categories, tokenizer, BERT_SEQUENCE_LEN,
                                             num_split)
inputs += compute_compact_split_input_arrays(df_train, qa_input_categories, tokenizer, BERT_SEQUENCE_LEN, num_split)

test_inputs = []
test_inputs += compute_compact_split_input_arrays(df_test, question_input_categories, tokenizer, BERT_SEQUENCE_LEN,
                                                  num_split)
test_inputs += compute_compact_split_input_arrays(df_test, qa_input_categories, tokenizer, BERT_SEQUENCE_LEN, num_split)
