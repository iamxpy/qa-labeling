...
def other_deal_result(all_predictions, ban_list=None):
    all_predictions_new = []
    idx_max = [0 for _ in range(30)]
    for i in range(len(all_predictions)):
        tem = []
...
classes = np.load('../input/google-quest-challenge/labels.npy', allow_pickle=True)
...
def align_result(predictions, ban_list=None):
