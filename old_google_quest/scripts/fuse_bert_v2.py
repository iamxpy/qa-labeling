...
def attention(query, key, value, mask=None, dropout=None, scaled=True):
    """
    code adapted from http://nlp.seas.harvard.edu/2018/04/03/attention.html
    """
    d_k = tf.cast(key.shape[-1], tf.float32)
    scores = tf.matmul(query, key, transpose_b=True) / (tf.math.sqrt(d_k) if scaled else 1.0)
    if mask is not None:
...
def multi_bert_model():
    ...
    attention_mask = tf.expand_dims(input_answer_masks, -2)  # add the "query_seq_len" dimension
    qa_sequence_output, _ = attention(query=question_sequence_output, key=answer_sequence_output,
                                      value=answer_sequence_output, mask=attention_mask, scaled=True)
