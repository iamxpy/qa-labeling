...
__author__ = 'qiaoyang'
__time__ = '2019/12/10 14:59'
__project__ = 'google-quest-challenge'
...
rare_words_mapping = {' s.p ': ' ', ' S.P ': ' ', 'U.s.p': '', 'U.S.A.': 'USA', 'u.s.a.': 'USA', 'U.S.A': 'USA',
                      ...
                      'Terroristan': 'terrorist', 'antibrahmin': 'anti brahmin', 'fuckboys': 'fuckboy',
                      'Fuckboys': 'fuckboy', 'Fuckboy': 'fuckboy', 'fuckgirls': 'fuck girls',
                      'fuckgirl': 'fuck girl', 'Trumpsters': 'Trump supporters', '4sixth': 'forty sixth',
                      'culturr': 'culture', 'weatern': 'western', '4fourth': 'forty fourth', 'emiratis': 'emirates',
                      'trumpers': 'Trumpster',
                      ...
                      'f***': 'fuck', 'f**': 'fuc', 'F***': 'fuck', 'F**': 'fuc', 'a****': 'assho', 'a**': 'ass',
...
bad_case_words = {'nationalpost': 'national post', 'businessinsider': 'business insider', 'jewprofits': 'jew profits',
                  'QMAS': 'Quality Migrant Admission Scheme', 'casterating': 'castrating',
                  ...
                  'trumpians': 'Trump supporters', 'Trumpster': 'trumpeters', 'irreputable': 'reputation',
                  'serieusly': 'seriously',
                  ...
                  'trumpsters': 'trumpeters', 'Trumpers': 'president trump', 'ciswomen': 'cis women',
                  'womenizer': 'womanizer',
                  ...
                  'humilates': 'humiliates', 'vicevice': 'vice vice', 'politicak': 'political',
                  'Sumaterans': 'Sumatrans',
                  'Kamikazis': 'Kamikazes', 'unmoraled': 'unmoral', 'eduacated': 'educated', 'moraled': 'morale',
                  'Amharc': 'Amarc', 'where Burkhas': 'wear Burqas', 'Baloochistan': 'Balochistan',
                  'durgahs': 'durgans',
                  'illigitmate': 'illegitimate', 'hillum': 'helium', 'treatens': 'threatens',
                  'mutiliating': 'mutilating',
                  'speakingly': 'speaking', 'pretex': 'pretext', 'menstruateion': 'menstruation',
                  'genocidizing': 'genociding', 'maratis': 'Maratism', 'Parkistinian': 'Pakistani',
                  'SPEICIAL': 'SPECIAL',
                  'REFERNECE': 'REFERENCE', 'provocates': 'provokes', 'FAMINAZIS': 'FEMINAZIS',
                  'repugicans': 'republicans',
                  'tonogenesis': 'tone', 'winor': 'win', 'redicules': 'ridiculous', 'Beluchistan': 'Balochistan',
                  'volime': 'volume', 'namaj': 'namaz', 'CONgressi': 'Congress', 'Ashifa': 'Asifa',
                  'queffing': 'queefing',
                  'montheistic': 'nontheistic', 'Rajsthan': 'Rajasthan', 'Rajsthanis': 'Rajasthanis',
                  'specrum': 'spectrum',
                  'brophytes': 'bryophytes', 'adhaar': 'Adhara', 'slogun': 'slogan', 'harassd': 'harassed',
                  'transness': 'trans gender', 'Insdians': 'Indians', 'Trampaphobia': 'Trump aphobia',
                  'attrected': 'attracted',
                  ...
                  'loundly': 'loudly', 'bootythongs': 'booty thongs', 'understamding': 'understanding',
                  'degenarate': 'degenerate',
                  'narsistic': 'narcistic', 'innerskin': 'inner skin', 'spectulated': 'speculated',
                  'hippocratical': 'Hippocratical',
...
def clean_text(x):
    x = str(x).replace(' s ', '').replace('…', ' ').replace('—', '-').replace('•°•°•',
                                                                              '')  # should be broken down to regexs (lazy to do it haha)
    for punct in "/-'":
        if punct in x:
            x = x.replace(punct, ' ')
    for punct in '&':
...
def bert_preprocess(df):
    ...
    # df["comment_text"] = df["comment_text"].progress_apply(lambda x: clean_text(x))
    # df['comment_text'] = df['comment_text'].apply(lambda x: clean_text(x))
    # df['comment_text'] = df['comment_text'].apply(lambda x: 'na' if x == "" else x)

    parallel = Parallel(48, backend="multiprocessing", verbose=0)
    print('bert_preprocess:')

    df['question_title'] = parallel(
        delayed(correct_contraction)(x, contraction_mapping) for x in df['question_title'].tolist())
    df['question_title'] = parallel(
        delayed(preprocess)(x) for x in df['question_title'].tolist())
    df['question_title'] = parallel(
        delayed(clean_text)(x) for x in df['question_title'].tolist())
    print('question_title done')

    df['question_body'] = parallel(
        delayed(correct_contraction)(x, contraction_mapping) for x in df['question_body'].tolist())
    df['question_body'] = parallel(
        delayed(preprocess)(x) for x in df['question_body'].tolist())
    df['question_body'] = parallel(
        delayed(clean_text)(x) for x in df['question_body'].tolist())
    print('question_body done')

    df['answer'] = parallel(
        delayed(correct_contraction)(x, contraction_mapping) for x in df['answer'].tolist())
    df['answer'] = parallel(
        delayed(preprocess)(x) for x in df['answer'].tolist())
    df['answer'] = parallel(
        delayed(clean_text)(x) for x in df['answer'].tolist())
    print('answer done')
