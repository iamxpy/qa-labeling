from src.roberta_datasets import *
    ...
    for cv in range(NFOLDS):
        ...
        for i, row in train_df.iterrows():
            if row['question_type_spelling'] > 0:
                a = train_df.loc[i]
                d = pd.DataFrame(a).T
                train_df_more = train_df_more.append([d] * 15)
            if row['question_not_really_a_question'] > 0:
                a = train_df.loc[i]
                d = pd.DataFrame(a).T
                train_df_more = train_df_more.append([d] * 4)
            if row['question_type_consequence'] > 0:
                a = train_df.loc[i]
                d = pd.DataFrame(a).T
                train_df_more = train_df_more.append([d] * 2)
        ...
        model = RobertaForQuest()
        swa_model = RobertaForQuest()
        ...
        for step in range(STEPS):
            ...
            # update swa model
            alpha = min(1 - 1 / (step + 1), swa_alpha)
            for swa_param, param in zip(swa_model.parameters(), model.parameters()):
                swa_param.data.mul_(alpha).add_(1 - alpha, param.data)
            ...
            if (step + 1) % eval_every == 0:
                ...
                swa_val_loss, swa_score, swa_post_score, swa_other_score, swa_align_score, swa_union_score, _, _, _, _ = validation(
                    swa_model, val_loader, loss_fn)
        ...
        average_swa_score += corr_swa_score
...
if cfg["Use_semi_supervised"]:
    ...
    sub[target_columns] = preds
    sub.to_csv("semi_{}".format(cfg["output"]), index=False)
