...
class CustomCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if self.fold is not None:
            ...
            self.model.save_weights(f'../output/models/fold-{fold}-epoch-{epoch}.h5')  # 一共保存fold * epoch个模型
...
def multi_split_bert_model():
    ...
    # [part_num*batch_size*num_split, BERT_SEQ_LEN] -> [part_num*batch_size*num_split, 768]
    pooled_output, sequence_output = bert([bert_ids, bert_masks, bert_segments])
    print(f"sequence_output.shape: {sequence_output.shape}")

    pooled_output = layers.Dropout(0.2)(pooled_output)

    # [part_num*batch_size*num_split, 768] -> [part_num*batch_size, num_split, 768]
    rnn_input = tf.reshape(pooled_output, [-1, num_split, pooled_output.shape[-1]], name="rnn_input")

    # [part_num*batch_size, num_split, 768] -> [part_num*batch_size, 2*rnn_hidden_size]
    rnn_output = layers.Bidirectional(layers.LSTM(lstm_hidden))(rnn_input)

    # [part_num*batch_size, 2*rnn_hidden_size] -> [part_num, batch_size, 2*rnn_hidden_size]  (part_num=2)
    split_output = tf.reshape(rnn_output, [2, -1, 2 * lstm_hidden])

    # [part_num, batch_size, 2*rnn_hidden_size] -> 2 x [batch_size, 2*rnn_hidden_size]
    question_rnn_output, qa_rnn_output = tf.unstack(split_output, axis=0)

    # question_fc_input = layers.Dense(128, activation="relu", name="question_fc_input")(question_rnn_output)
    # qa_fc_input = layers.Dense(128, activation="relu", name="qa_fc_input")(qa_rnn_output)

    # [batch_size, rnn_hidden_size] -> [batch_size, output_size]
    question_out = layers.Dense(21, activation="sigmoid", name="question_output")(question_rnn_output)
    qa_out = layers.Dense(9, activation="sigmoid", name="qa_output")(qa_rnn_output)
...
train_rho_df.to_csv(f'../output/models/train-rho-start-{start_fold}.csv', index=False)
# pred_train_df = pd.DataFrame(pred_train, columns=output_categories)
# pred_train_df.to_csv(f'../output/models/pred-train-{start_fold}.csv', index=False)
