...
df_train = pd.read_csv(PATH + 'train.csv').head(3000)
...
class CustomCallback(tf.keras.callbacks.Callback):
    ...
    def on_train_begin(self, logs={}):
        ...
        print("step  epoch  train_loss | rho  swa_rho | time")
        print('-' * 100)
    ...
    def on_batch_end(self, batch, logs={}):
        ...
        if batch > 0 and batch % self.eval_every == 0:
            ...
            rho_val = compute_spearmanr(self.valid_outputs, val_pred)
            ...
            # check whether to save model and update best rho value todo 改为用swa_val做指标
            if rho_val > self.rho_value:
                self.rho_value = rho_val
                self.model.save_weights(f'{self.output_path}/fold-{fold}-best.h5')

            elapsed_time = time.time() - self.start_time
            print("\n%.1f  %.1f  %.4f | %.4f  %.4f | %.4f seconds" % (
            batch / 1e3, batch / self.epoch_steps, logs.get('loss'), rho_val, swa_rho_val, elapsed_time))
