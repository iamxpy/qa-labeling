...
def qa_trim_input(tokenizer, title, body, answer):
    all_title = []
    all_body = []
    all_answer = []
    for t, b, a in zip(title, body, answer):
        ...
        if (t_len + b_len + a_len + 4) > MAX_LEN:
            ...
            if t_new_len + a_new_len + b_new_len + 4 != MAX_LEN:
                raise ValueError("New sequence length should be %d, but is %d" % (
                MAX_LEN, (t_new_len + a_new_len + b_new_len + 4)))
            ...
            if cfg["choose"] == "head":
                ...
                assert b_new_len == b_head_len + b_tail_len
...
def q_trim_input(tokenizer, title, body):
    all_title = []
    all_body = []
    for t, b in zip(title, body):
        ...
        if (t_len + b_len + 3) > MAX_LEN:

            if t_max_len > t_len:
                t_new_len = t_len
                b_max_len = MAX_LEN - 3 - t_new_len
            else:
                t_new_len = t_max_len
            ...
            if t_new_len  + b_new_len + 3 != MAX_LEN:
                raise ValueError("New sequence length should be %d, but is %d" % (
                MAX_LEN, (t_new_len + b_new_len + 3)))
...
def convert_lines(tokenizer, title, body, answer):
    ...
    for i, (q_t, q_b, t, b, a) in enumerate(zip(q_title, q_body, qa_title, qa_body, qa_answer)):
        q_stoken = ["[CLS]"] + q_t + ["[SEP]"] + q_b + ["[SEP]"]

        q_ids = get_ids(q_stoken, tokenizer, MAX_LEN)
        q_masks = get_masks(q_stoken, MAX_LEN)
        q_segments = q_get_segments(q_stoken, MAX_LEN)
        q_input_ids.append(q_ids)
        q_input_masks.append(q_masks)
        q_input_segments.append(q_segments)

        qa_stoken = ["[CLS]"] + t + ["[SEP]"] + b + ["[SEP]"] + a + ["[SEP]"]
        qa_ids = get_ids(qa_stoken, tokenizer, MAX_LEN)
        qa_masks = get_masks(qa_stoken, MAX_LEN)
        qa_segments = qa_get_segments(qa_stoken, MAX_LEN)
